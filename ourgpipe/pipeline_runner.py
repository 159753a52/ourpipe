...
使用方法:
    ...
    # 1F1B
    torchrun --nproc_per_node=4 --master_port=29500 pipeline_runner.py --config configs/gpt_small_1f1b.yaml

    # ZeroBubble
    torchrun --nproc_per_node=4 --master_port=29500 pipeline_runner.py --config configs/gpt_small_zerobubble.yaml

    # Hanayo
    torchrun --nproc_per_node=4 --master_port=29500 pipeline_runner.py --config configs/gpt_small_hanayo.yaml

    # 命令行覆盖调度器
    torchrun --nproc_per_node=4 --master_port=29500 pipeline_runner.py --config configs/gpt_small.yaml --scheduler 1f1b
    ...
    LD_PRELOAD=/home/bingxing2/home/scx9kvs/orion-docker/kernel_intercept/build/libgpu_scheduler.so \
        torchrun --nproc_per_node=4 --master_port=29500 pipeline_runner.py --config configs/gpt_small_orion.yaml
"""
...
def main():
    ...
    parser.add_argument('--scheduler', type=str, default=None, 
                        help='Override scheduler type (naive, async_threaded, 1f1b, hanayo, zerobubble)')
    ...
    # ===== 根据调度器类型选择 Stage 类 =====
    stage_name = config.model.name
    if config.parallel.scheduler == 'hanayo':
        stage_name = f"{config.model.name}_hanayo"
    
    stage_cls = STAGE_REGISTRY.get(stage_name)
    if stage_cls is None:
        raise ValueError(
            f"Stage '{stage_name}' not found. Available: {STAGE_REGISTRY.list_registered()}. "
            f"For hanayo scheduler, make sure '{config.model.name}_hanayo' stage is registered."
        )
    ...
    # ===== ZeroBubble: 替换 nn.Linear 为 BubbleLinear =====
    if config.parallel.scheduler == 'zerobubble':
        from core.bubble_linear import convert_to_bubble_model
        convert_to_bubble_model(my_stage.sub_model)
        if global_rank == 0:
            print("ZeroBubble: converted nn.Linear to BubbleLinear in sub_model")
    ...
    is_hanayo = (config.parallel.scheduler == 'hanayo')
    total_iterations = 0
    ...
    for epoch in range(config.training.epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), disable=(global_rank != 0)):
            ...
            # ===== 根据调度器类型拆分 micro-batch =====
            if is_hanayo:
                # Hanayo 需要 2x microbatch：前半 Wave A，后半 Wave B
                micro_inputs = list(torch.chunk(inputs, num_microbatches * 2))
                micro_labels = list(torch.chunk(labels, num_microbatches * 2))
            else:
